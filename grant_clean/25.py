...
def clean_row(row) :
    ...
    features["country"] = "IN"
    features["researcher"] = clear_res_ins(features["researcher"])
    features["institution"] = clear_res_ins(features["institution"])
...
def multi_process_db(grant_sites_id) :
    ...
    for row in query_result :
        ...
        if features["id"] not in hash_id :
            sub_grant_datas.append(features)
            hash_id[features["id"]] = 1
