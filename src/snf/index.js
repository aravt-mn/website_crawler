...
async function crawl(browser, query, params, startDate) {
  try {
    ...
    if (query.request_type === 'SNF') combinations = listUrlCombinations;
        ...
        try {
                ...
                if (query.response_type === 'HTML') {
                  try {
                    await listPage.evaluate(() => {
                      const list = document.getElementsByClassName('click');
                      for (const l of list) {
                        l.click();
                      }
                    });
          ...
          if (response) {
            ...
            if (query.response_type === 'HTML') {
              ...
              const text = await $('tr#ctl00_MainContent_trProjectFundingInstrument').first().text();
              if (!text || (!text.toUpperCase().includes('Project funding'.toUpperCase()) && !text.toUpperCase().includes('Projektförderung'.toUpperCase()))) {
