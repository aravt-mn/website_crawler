...
async function getCrawledSites(site_id, startDate) {
  const connection = createConnection();
  const result = [];
  const query = 'SELECT project_url FROM grant_crawled_new WHERE grant_sites_id = $1 and crawled_date > $2';
  const urls = (await runQuery({
    connection,
    query,
    params: [site_id, startDate],
  })).rows;

  for (const url of urls) {
    if (url.project_url.split('filename').length > 1) {
      result[result.length] = url.project_url.split('filename')[1];
    }
  }
  return result;
}
...
async function isCrawledUrl(dtlLink, startDate, crawledList) {
  const str = dtlLink.split('filename').length > 1 ? dtlLink.split('filename')[1] : dtlLink;
  if (crawledList.includes(str)) {
    return true;
  }
  const connection = createConnection();
  // get query params
  const query = `SELECT a.* FROM grant_crawled_new a WHERE a.project_url like '%${str}%' AND a.crawled_date > $1 LIMIT 1`;
...
async function clickSeeMore(selector, page){
    try{
...
async function crawlInfo(
  ...
  try {
    ...
    // is crawled link
    if (await isCrawledUrl(dtlUrl, startDate, crawledList) && query.detail_path) {
    ...
    for (let tryCount = 0; tryCount < 5; tryCount += 1) {
      try {
        ...
        if (dtlUrl) {
          ...
          const seeMoreIds = ["#ChDivSummaryMore","#ChDivSummaryMore2"];

          for(selector of seeMoreIds){
            await clickSeeMore(selector, dtlPage);
          }
          // eslint-disable-next-line no-await-in-loop
